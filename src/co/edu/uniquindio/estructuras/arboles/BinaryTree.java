...
public class BinaryTree<E extends Comparable<E>> {
	...
	/**
	 * Main method for adding a new value to the tree following the rules of a binary
	 * tree
	 * @param value
	 */
	...
	/**
	 * Method that recursively traverses the tree to its leafs, checks if the value is the current node value, 
	 * if not it searches the left side, if it is not found in all the left side it proceeds 
	...
	/**
	 * Method that recursively traverses the tree obtaining the number of nodes on the left 
	 * and then on the right adding one to add the current node to the quantity.
	 * @param node
	 * @return
	 */
	...
	/**
	 * Main method to get the height of a tree
	 ...
	 * command 1 at each node to the highest, 
	 * the null nodes return 0 so that the leafs 
	 * can return 1 and make the height the longest path plus 1.
	 * @param node
	 * @return
	 */
	...
	/**
	 * Busca por todo el arbol el nodo, y va sumando su nivel por cada izquierda 
	 * o derecha que se haga para encontrarlo, y si llega a una hoja retorna -1, para que
	 * con el encontrado el nivel sea mayor
	 * @param node
	 * @param value
	 * @param i
	 * @return
