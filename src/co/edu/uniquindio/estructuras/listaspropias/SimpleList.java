...
public class SimpleList<E> implements Iterable<E> {
	...
	/**
	 * Metodo para agregar una lista en un indice de otroa lista
	 * @param indice
	 * @param listaAgregar
	 */
	public void agregarLista(int indice, SimpleList<E>listaAgregar) {
		int index=0;
		if(head==null) {
			head=listaAgregar.head;
			return;
		}
		...
		SimpleNode<E> aux= current.getNext();
		current.setNext(listaAgregar.head);
		while(current.getNext()!=null) {
			current=current.getNext();
		}
		current.setNext(aux);
	}
	public void agregarListaMain(int indice,SimpleList<E>listaAgregar) {
		if(indice<-1 ||indice>size|| listaAgregar.head==null) {
			throw new RuntimeException("No se pudo");
		}else {
			agregarLista(indice, listaAgregar);
		}
	}
